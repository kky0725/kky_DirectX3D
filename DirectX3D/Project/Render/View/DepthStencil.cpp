...
void DepthStencil::CreateDSVTexture()
{
	...
	DEVICE->CreateTexture2D(&depthDesc, nullptr, &_dsvTexture);
}
...
void DepthStencil::CreateDSV()
{
	...
	DEVICE->CreateDepthStencilView(_dsvTexture, &dsvDesc, &_dsv);
}
...
void DepthStencil::CreateSRV()
{
	...
	DEVICE->CreateShaderResourceView(_dsvTexture, &srvDesc, &_srv);
}
