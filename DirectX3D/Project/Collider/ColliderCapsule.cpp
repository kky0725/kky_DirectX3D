...
bool ColliderCapsule::Collision(IN Ray& ray, OUT Contact* contact)
{
	...
	Vector3 pa = _globalPosition - direction * Height() * 0.5f;
	Vector3 pb = _globalPosition + direction * Height() * 0.5f;
...
{
	Vector3 direction = Up();
	Vector3 A = _globalPosition - direction * Height() * 0.5f;
	Vector3 B = _globalPosition + direction * Height() * 0.5f;

	Vector3 P = other->_globalPosition;
...
bool ColliderCapsule::Collision(ColliderCapsule* other)
{
	...
	Vector3 aA = _globalPosition - aDirection * Height() * 0.5f;
	Vector3 aB = _globalPosition + aDirection * Height() * 0.5f;

	Vector3 bDirection = other->Up();
	Vector3 bA = other->_globalPosition - bDirection * other->Height() * 0.5f;
	Vector3 bB = other->_globalPosition + bDirection * other->Height() * 0.5f;
